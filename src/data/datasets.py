...
class ODDataset():
    ...
    def read_and_transform(self, img_fname, label_fname, transforms=None, idx=None):
        ...
        target = {
            ...
            'labels': torch.as_tensor(transformed['label_ids'], dtype=torch.int64) if transformed['label_ids'] else torch.zeros((0, ), dtype=torch.int64), # Corresponding class IDs
            'label_names': transformed['label_names'] if transformed['label_names'] else torch.zeros((0, ), dtype=torch.int64) # Corresponding class names
        }
